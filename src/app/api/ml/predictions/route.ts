// ============================================
// RECO-TRADING - ML Predictions API
// ============================================
// GET /api/ml/predictions
// Returns ML prediction, accuracy, history,
...
export async function GET() {
  ...
  try {
    ...
    // Try to fetch real prediction history from DB
    let dbHistory: any[] = [];
    ...
    try {
      ...
      // DB not ready, use generated history
    }

    // Use DB history if available, otherwise use generated
    const history = dbHistory.length > 0 ? dbHistory : result.history;

    // Calculate real accuracy from DB if available
    ...
    try {
      ...
      // DB calculations not available
    }

    // Save this prediction to DB
    try {
