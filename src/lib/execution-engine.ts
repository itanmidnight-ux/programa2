...
export class ExecutionEngine {
  ...
  async tick(): Promise<TickResult> {
    ...
    try {
      ...
      // 2. Run analysis â€” adapt OrderBook to expected format
      ...
      if (!this.currentPosition) {
        ...
        if (ensemble.finalSignal === 'NEUTRAL') {
          if (this.tickCount % 6 === 0) {
            console.log(`[ENGINE] NEUTRAL signal â€” score: ${ensemble.weightedScore.toFixed(3)}, confidence: ${ensemble.confidence.toFixed(2)}, reasons: ${ensemble.reasons.slice(0, 3).join('; ')}`);
          }
        } else {
          ...
          if (burstEval.shouldBurst) {
            ...
            console.log(
              `[ENGINE] ðŸš€ STRONG SIGNAL detected: ${burstEval.signalStrength.level} ` +
            ...
            if (burstResult.triggered) {
              ...
              console.log(`[ENGINE] ðŸ“Š Burst result: ${burstResult.tradesExecuted} executed, ${burstResult.tradesFailed} failed`);
            } else {
  ...
  async closePosition(reason: string): Promise<TradeResult> {
    ...
    try {
      // Place real closing order on Broker (only for non-dry-run trades)
      if (!this.config.dryRun && isBrokerConnected()) {
        ...
        if (!closeResult.success) {
          ...
          // The position may still be open on the exchange â€” user should check manually
        } else {
  ...
  async managePosition(currentPrice: number, analysis: FullAnalysis): Promise<PositionAction | null> {
    ...
    // 0.3%+ profit after 15s â†’ scalp
    if (profitPct >= 0.3 && timeOpenSec >= 15) {
      console.log(`[ENGINE] ðŸ’° Scalp: ${profitPct.toFixed(2)}% after ${timeOpenSec.toFixed(0)}s`);
      return { type: 'CLOSE', message: `Scalp ${profitPct.toFixed(2)}%` };
    }
    // 0.8%+ profit â†’ close on weakness or 30s+
    if (profitPct >= 0.8 && (timeOpenSec >= 30 || analysis.rsi > 65 || analysis.rsi < 35)) {
      console.log(`[ENGINE] ðŸ’° Secure: ${profitPct.toFixed(2)}% after ${timeOpenSec.toFixed(0)}s`);
      return { type: 'CLOSE', message: `Secure ${profitPct.toFixed(2)}%` };
    }
    // 2%+ profit â†’ aggressive close
    if (profitPct >= 2.0) {
      console.log(`[ENGINE] ðŸ’°ðŸ’° Strong profit: ${profitPct.toFixed(2)}%`);
      return { type: 'CLOSE', message: `Lock ${profitPct.toFixed(2)}%` };
    }
    // -1% loss after 20s â†’ cut loss
    if (profitPct <= -1.0 && timeOpenSec >= 20) {
      console.log(`[ENGINE] ðŸ›‘ Cut loss: ${profitPct.toFixed(2)}% after ${timeOpenSec.toFixed(0)}s`);
    ...
    switch (stopAction.type) {
      ...
      case 'CLOSE_PARTIAL': {
        ...
        // Place real partial closing order on Broker (only for non-dry-run trades)
        if (!this.config.dryRun && isBrokerConnected()) {
          const closeSide = pos.side === 'LONG' ? 'SELL' : 'BUY';
  ...
  validateEntry(signal: { direction: string; confidence: number; sl: number; tp: number; reasons: string[] }, analysis: FullAnalysis): { valid: boolean; reason?: string } {
    ...
    // Check minimum confidence â€” use the LOWER of market-adapted vs configured
