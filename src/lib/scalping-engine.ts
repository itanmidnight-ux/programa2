...
// CAPA 2: Optimizador de ejecuciÃ³n (Market Making direccional)
// CAPA 3: Filtro inteligente de mercado
// CAPA 4: GestiÃ³n dinÃ¡mica de riesgo
// ============================================

import { getKlines, getOrderBook, getTickerPrice, placeLimitOrder, cancelOrder, getOrderStatus, isTestnetMode, getCurrentCredentials } from '@/lib/market-gateway';
...
// ============================================
// CONFIGURACIÃ“N Ã“PTIMA (lista para producciÃ³n)
// ============================================

const TRADE_CONFIG = {
  // Fees (Broker USDT-M futures - considerar maker)
  MAKER_FEE: 0.0002,    // 0.02%
  TAKER_FEE: 0.0004,    // 0.04%
  SLIPPAGE: 0.0003,     // 0.03% estimated (LIMIT orders son mejores)
  
  // TP/SL percentages (no ATR-based)
  TP_MIN_PERCENT: 0.003,  // 0.3% mÃ­nimo
  TP_MAX_PERCENT: 0.008,  // 0.8% mÃ¡ximo
  SL_MIN_PERCENT: 0.002,  // 0.2% mÃ­nimo
  SL_MAX_PERCENT: 0.004,  // 0.4% mÃ¡ximo
  ...
  // Market filters
  MIN_ATR_PERCENT: 0.2,    // MÃ­nima volatilidad
  MAX_ATR_PERCENT: 3.0,    // MÃ¡xima volatilidad (evitar spikes)
  MIN_VOLUME_RATIO: 0.6,   // Volumen mÃ­nimo vs media
  MAX_SPREAD_PERCENT: 0.15,// Spread mÃ¡ximo
  ...
  MIN_CONFIDENCE: 0.40,
  MIN_RISK_REWARD: 1.5,        // MÃ­nimo 1.5x
  ...
  GRID_SIZE_MULTIPLIER: 0.5,  // Reducir tamaÃ±o en grid
  
  // Trailing stop
  TRAILING_ACTIVATION: 0.5,   // Activar cuando TP% alcanzado
  TRAILING_DISTANCE: 0.002,   // 0.2% detrÃ¡s
...
export class AdaptiveModeSystem {
  ...
  evaluateAndSwitch(
    ...
    if (newMode !== oldMode) {
      logger.opt(`Mode changed: ${oldMode} â†’ ${newMode} | WR: ${(winRate*100).toFixed(0)}% | DD: ${currentDrawdown.toFixed(1)}% | ATR: ${atrPercent.toFixed(1)}%`);
    }
...
export function generateScalpingSignal(candles: Candle[], analysis: any): ScalpingSignal {
  ...
  // 3. Precio cerca de EMA (evitar extensiÃ³n)
  ...
  if (analysis.volumeRatio > 1.2) {
    ...
    reasons.push('Bajo volumen - seÃ±al dÃ©bil');
  }
  
  // 5. Ajuste por rÃ©gimen de mercado
  ...
  // TP/SL basados en porcentaje determinista (basado en rÃ©gimen de mercado y volatilidad)
...
async function calculateOptimalOffset(
  ...
  if (!orderBook || orderBook.bids.length === 0 || orderBook.asks.length === 0) {
    // En tendencias, colocar mÃ¡s cerca del precio
    if (regime === 'TRENDING_UP' || regime === 'TRENDING_DOWN') {
      return 0.0002;
    }
    return 0.0005;
  ...
  while (Date.now() - startTime < timeoutMs) {
    try {
      ...
      if (orderStatus === 'FILLED') {
        const filledPrice = parseFloat(String(status.price ?? originalPrice));
        return {
          success: true,
          orderId: String(orderId),
          filledPrice: filledPrice,
          filledQty: parseFloat(String(status.executedQty ?? quantity)),
          message: `FILLED @ ${filledPrice}`,
        };
      }
      
      if (orderStatus === 'PARTIALLY_FILLED') {
        const filledQty = parseFloat(String(status.executedQty ?? 0));
        if (filledQty > 0) {
          const avgPrice = parseFloat(String(status.price ?? originalPrice));
          return {
            success: true,
            orderId: String(orderId),
...
// ============================================
// RISK MANAGER CON GESTIÃ“N DINÃMICA (CAPA 4)
// ============================================
...
export class ScalpingRiskManager {
  ...
  getStatus(): RiskStatus {
    ...
    // Aumentar tamaÃ±o en racha positiva
    ...
    return {
      ...
      reason: isPaused 
        ? `Pausado: ${consecutiveLosses} pÃ©rdidas, ${Math.ceil((TRADE_CONFIG.PAUSE_ON_LOSS_MINUTES * 60 * 1000 - timeSincePause) / 60000)}min restantes`
        : dailyLossPercent <= -TRADE_CONFIG.DAILY_LOSS_LIMIT_PERCENT 
          ? `LÃ­mite pÃ©rdida diaria: ${dailyLossPercent.toFixed(2)}%`
          : drawdown >= TRADE_CONFIG.MAX_DRAWDOWN_TO_PAUSE
            ? `Drawdown mÃ¡ximo: ${drawdown.toFixed(2)}%`
            : 'OK',
