...
async function loadOrGetModel(
  ...
  // Load from DB
  console.log(`ðŸ” Loading model ${cacheKey}...`);
...
export async function predictTrade(
  ...
  try {
    console.log(`ðŸ¤– Predicting for ${input.pair}/${input.timeframe}...`);
    ...
    console.log(`âœ“ Prediction: ${prediction} (confidence: ${(confidence * 100).toFixed(2)}%)`);
    ...
    console.error(`âŒ Error making prediction: ${error}`);
    throw error;
  }
}
...
export function disposeAllModels(): void {
  ...
  modelCache.clear();
  console.log('âœ“ All models disposed from memory');
}
