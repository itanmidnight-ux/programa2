...
// ============================================
// Entrena redes neuronales especÃ­ficas por par/timeframe
// usando TensorFlow.js y datos histÃ³ricos de Broker
...
export async function prepareTrainingData(
  ...
  console.log(`ðŸ“Š Extracting ${FEATURE_NAMES.length} features...`);
  const features = extractFeatures(candles);
  const labels = generateLabels(candles);

  console.log(`âœ“ Extracted ${features.length} training examples`);
...
export async function trainModel(
  ...
  console.log(`\nðŸ¤– Training model for ${finalConfig.pair}/${finalConfig.timeframe}...`);
...
export async function trainCompleteModel(config: TrainingConfig): Promise<{
  ...
  try {
    ...
    console.log(`\nðŸ“¥ Downloading data for ${config.pair}/${config.timeframe} (last ${config.lookbackDays} days)...`);
    ...
    console.log(`âœ“ ${candles.length} candles downloaded`);
    ...
    console.error(`âŒ Error training model: ${error}`);
    throw error;
  }
}
