...
export async function runBacktest(config: BacktestConfig): Promise<BacktestResult> {
  console.log(`\n📊 Starting backtest for ${config.pair}/${config.timeframe}`);
  ...
  try {
    ...
    console.log(`✓ ${backtestCandles.length} candles loaded`);
    ...
    if (!passed) {
      console.warn(`⚠️  Strategy did NOT pass validation:`);
      notes.forEach(note => console.warn(`   - ${note}`));
    } else {
      console.log(`✓ Strategy passed all validation criteria`);
    }
...
function validateStrategy(result: BacktestResult): { passed: boolean; notes: string[] } {
  ...
  if (result.totalTrades < CRITERIA.minTrades) {
    notes.push(`⚠️  Too few trades (${result.totalTrades} < ${CRITERIA.minTrades})`);
    passed = false;
  }

  if (result.winRate < CRITERIA.minWinRate) {
    notes.push(`❌ Low win rate (${(result.winRate * 100).toFixed(2)}% < ${CRITERIA.minWinRate * 100}%)`);
    passed = false;
  }

  if (result.profitFactor < CRITERIA.minProfitFactor) {
    notes.push(`❌ Low profit factor (${result.profitFactor.toFixed(2)} < ${CRITERIA.minProfitFactor})`);
    passed = false;
  }

  if (result.maxDrawdownPercent > CRITERIA.maxDrawdown) {
    notes.push(`❌ High drawdown (${result.maxDrawdownPercent.toFixed(2)}% > ${CRITERIA.maxDrawdown}%)`);
    passed = false;
  }

  if (result.sharpeRatio < CRITERIA.minSharpeRatio) {
    notes.push(`⚠️  Low Sharpe ratio (${result.sharpeRatio.toFixed(2)} < ${CRITERIA.minSharpeRatio})`);
  }

  // Positive validations
  if (result.winRate >= 0.65) {
    notes.push(`✅ Excellent win rate (${(result.winRate * 100).toFixed(2)}%)`);
  }

  if (result.profitFactor >= 2.0) {
    notes.push(`✅ Very good profit factor (${result.profitFactor.toFixed(2)})`);
  }

  if (result.maxDrawdownPercent <= 10) {
    notes.push(`✅ Controlled drawdown (${result.maxDrawdownPercent.toFixed(2)}%)`);
  }
...
export async function compareStrategies(
  ...
  for (const result of results) {
    const status = result.passedValidation ? '✅ PASS' : '❌ FAIL';
